...
/**
 * D의 심장 — 게이트키핑 순수 함수 (명세 forD 2절).
 *
 * 부작용이 전혀 없다. 오직 (상태, commandId)만 보고 판정하므로 5×17=85 조합을 유닛 테스트로
...
object CommandGate {
    ...
    private fun reasonFor(state: ControllerState, commandId: CommandId): RejectReason = when {
        ...
        state == ControllerState.DRAGGING &&
            commandId !in DRAG_RELATED ->
            RejectReason.DRAG_IN_PROGRESS

        else -> RejectReason.INVALID_IN_STATE
    }

    // DRAGGING에서 유효하거나 안전 처리되는 명령들(이 외 명령이 오면 "드래그 중" 안내)
    private val DRAG_RELATED = setOf(CommandId.DRAG_END, CommandId.DRAG_CANCEL, CommandId.STOP)
}
