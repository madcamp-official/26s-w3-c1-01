 ...
 *
 * 제어 명령(LOCK/UNLOCK)은 OS 이벤트가 아니므로 ExecutionCommand로
 * 내려보내지 않는다. 실제 OS 탭/뒤로가기/스와이프가 필요한 명령만 여기로 전달된다.
 *
 * 스크롤 방향, 좌표 등 실행에 필요한 부가 정보는 [payload]로 전달한다.
 * payload 키 목록은 통합 시 C와 합의한다(명세 forD 9절).
 *
 * @param commandId 실행할 명령.
